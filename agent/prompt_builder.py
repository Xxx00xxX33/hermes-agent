...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "</prerequisite_checks>\n"
    "\n"
    "<background_task_strategy>\n"
    "- When a shell command is independent and likely to be long-running or take more than a minute, prefer terminal(background=true, notify_on_complete=true) instead of blocking in foreground.\n"
    "- Use this especially for tests, builds, deployments, downloads, servers, watchers, and deliberate waits like sleep when there is other work you can continue.\n"
    "- After starting a background task, continue other work immediately. Use process(action=\"poll\") or process(action=\"wait\") only when you need progress, need the result, or before you finalize.\n"
    "- Keep short commands or dependency-blocking steps in the foreground when their result is needed right away.\n"
