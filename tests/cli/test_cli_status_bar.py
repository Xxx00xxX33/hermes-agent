...
class TestCLIStatusBar:
    ...
    def test_build_status_bar_text_places_subtask_progress_last_on_wide_terminal(self):
        ...
        cli_obj._subagent_task_counter = 5
        cli_obj._subagent_active_tasks = {"sa-1": object(), "sa-2": object(), "sa-3": object()}
    ...
    def test_status_bar_primary_line_uses_named_custom_provider_and_subtask_progress(self):
        cli_obj = _attach_todo_items(
            _attach_agent(
                _make_cli(model="DeepSeek-R1"),
                prompt_tokens=10_230,
    ...
    def test_status_bar_primary_line_keeps_provider_visible_with_todo_progress_when_no_subtasks_exist(self):
        cli_obj = _attach_todo_items(
            _attach_agent(
...
class TestToolProgressRefresh:
    def test_delegate_task_start_and_complete_updates_subtask_counts(self):
        cli_obj = _make_cli(model="gpt-5.4")
        cli_obj._status_bar_visible = True
        cli_obj._app = SimpleNamespace(invalidate=MagicMock())
        ...
        with patch("time.monotonic", side_effect=[100.0]):
            cli_obj._on_tool_start(
                "call-1",
                "delegate_task",
                {"tasks": [{"goal": "a"}, {"goal": "b"}, {"goal": "c"}]},
            )

        snapshot = cli_obj._get_status_bar_subagent_task_snapshot()
        assert snapshot == {"total": 3, "completed": 0, "active": 3}

        cli_obj._on_tool_complete("call-1", "delegate_task", {"tasks": [{"goal": "a"}]}, "done")
        snapshot = cli_obj._get_status_bar_subagent_task_snapshot()
        assert snapshot == {"total": 3, "completed": 3, "active": 0}

    def test_tool_completed_forces_immediate_status_refresh(self):
        cli_obj = _make_cli()
        ...
        cli_obj.tool_progress_mode = "none"
        cli_obj._emit_persistent_tool_progress_line = lambda *args, **kwargs: None
        cli_obj._app = SimpleNamespace(invalidate=MagicMock())

        with patch("time.monotonic", side_effect=[100.0, 100.0, 100.001]):
            cli_obj._on_tool_progress("tool.started", function_name="todo", preview="todo", function_args={})
            cli_obj._on_tool_progress("tool.completed", function_name="todo")

        assert cli_obj._app.invalidate.call_count == 2
