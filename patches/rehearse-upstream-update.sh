...
Environment:
  HERMES_LOCAL_CUSTOMIZATION_TMPDIR  Parent directory for the temporary rehearsal clone
                                    (defaults to $TMPDIR, then /tmp).
EOF
}
...
patch_state_in_repo() {
  ...
  elif git -C "$repo_root" apply --check "$patch_file" >/dev/null 2>&1; then
    printf 'applicable'
...
resolve_temp_root() {
  if [[ -n "${HERMES_LOCAL_CUSTOMIZATION_TMPDIR:-}" ]]; then
    printf '%s\n' "$HERMES_LOCAL_CUSTOMIZATION_TMPDIR"
  elif [[ -n "${TMPDIR:-}" ]]; then
    printf '%s\n' "$TMPDIR"
  else
    printf '/tmp\n'
...
REHEARSAL_TMP_ROOT="$(resolve_temp_root)"
mkdir -p "$REHEARSAL_TMP_ROOT"
REHEARSAL_TMPDIR="$(mktemp -d "$REHEARSAL_TMP_ROOT/hermes-upstream-rehearsal.XXXXXX")"
CLEAN_REPO="$REHEARSAL_TMPDIR/upstream"
cleanup() {
  if (( KEEP_TEMP == 0 )); then
    rm -rf "$REHEARSAL_TMPDIR"
  fi
}
trap cleanup EXIT
